...
for pid, path in enumerate(matfiles):
  ...
  training = pd.concat([training, pd.DataFrame(participant.training)], ignore_index = True)
  generation = pd.concat([generation, pd.DataFrame(participant.generation)], ignore_index = True)
  ranges = pd.concat([ranges, pd.DataFrame(stats['ranges'])], ignore_index = True)
  distances = pd.concat([distances, pd.DataFrame(stats['distances'])], ignore_index = True)
